    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] argString)
    {
        ...
        if (!mcWorld.isRemote) // Server side
        {
            ...
            if (argString == null || argString.length == 0)
            {
                ...
                if(isOp)
                {
                    sender.sendMessage(new TextComponentString(MESSAGE_COLOR + "/otg tp <biome name or id / dimension name> " + VALUE_COLOR + "Teleport to the given dimension or to the nearest biome with the given name or id (max distance 16000 blocks)."));
