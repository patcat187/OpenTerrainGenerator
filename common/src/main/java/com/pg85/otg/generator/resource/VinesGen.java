            ...
            if (worldMaterial != null && worldMaterial.isAir())
            {
                for (int direction = 2; direction <= 5; direction++)
                {
                    if (canPlace(world, _x, y, _z, direction, chunkBeingPopulated))
                    {
                        ...
                        //world.setBlock(_x, y, _z, LocalMaterialManager.toLocalMaterialData(LocalMaterialManager.VINE, 1 << D[OPPOSITE_FACING[direction]]), null, chunkBeingPopulated);
                    	world.setBlock(_x, y, _z, LocalMaterials.VINE, null, chunkBeingPopulated);
                        break;
                    }
                }
            } else {
