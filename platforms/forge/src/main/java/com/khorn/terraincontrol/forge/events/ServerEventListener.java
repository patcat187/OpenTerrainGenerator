...
{	
	...
	{
		ForgeWorld playerWorld = (ForgeWorld)((ForgeEngine)TerrainControl.getEngine()).getWorld(player.worldObj);
		...
		if(playerWorld != null && worldConfig.dimensionBelow != null && worldConfig.dimensionBelow.trim().length() > 0)
		{
			if(player.getPosition().getY() < worldConfig.dimensionBelowHeight)
			{
				...
				if(destinationWorld != null) // Dimension does not exist
				{
					...
					{
						player.worldObj.setBlockToAir(new BlockPos(player.getPosition().getX(), 254, player.getPosition().getZ()));
						player.worldObj.setBlockToAir(new BlockPos(player.getPosition().getX(), 255, player.getPosition().getZ()));
