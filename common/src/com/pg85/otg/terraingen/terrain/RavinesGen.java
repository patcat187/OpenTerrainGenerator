package com.pg85.otg.terraingen.terrain;
...
public class RavinesGen extends TerrainGenBase
{
            ...
            for (int localX = k; localX < m; localX++)
            {
                ...
                for (int localZ = i2; localZ < i3; localZ++)
                {
                    ...
                    if (d9 * d9 + d10 * d10 < 1.0D)
                    {
                        ...
                        {
                            d11 = ((currentDepth - 1) + 0.5D - paramDouble2) / d4;
                            if ((d9 * d9 + d10 * d10) * this.a[currentDepth - 1] + d11 * d11 / 6.0D < 1.0D)
                            {
                                ...
                                if (
                            		...
                            		(
										...
										block != DefaultMaterial.MAGMA_BLOCK &&
										!block.isAir()
                    				)
                                )
                                {
    ...
    protected void generateChunk(ChunkCoordinate currentChunk, ChunkBuffer generatingChunkBuffer)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            f3 = (this.random.nextFloat() * 2.0F + this.random.nextFloat()) * 2.0F;

            size = RandomHelper.numberInRange(random, this.worldSettings.ravineMinLength, this.worldSettings.ravineMaxLength);
