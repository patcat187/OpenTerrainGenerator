...
final class TXCommandHandler implements ICommand
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] argString)
    {
        ...
        boolean isOp = sender.canCommandSenderUseCommand(2, this.getCommandName());
                    ...
                    try
                    {
		            	...
		            	for(EnumCreatureType creatureType : EnumCreatureType.values())
		            	{
			    			...
			    			if(creatureList != null && creatureList.size() > 0)
			    			{
			    				...
			    				{
			    					sender.addChatMessage(new TextComponentTranslation(VALUE_COLOR + "{\"mob\": \"" + MobNames.toInternalName(spawnListEntry.entityClass.getSimpleName()) + "\", \"weight\": " + spawnListEntry.itemWeight + ", \"min\": " + spawnListEntry.minGroupCount + ", \"max\": " + spawnListEntry.maxGroupCount + "}"));
			    				}
			    			}
		            	}
	                }
