package com.pg85.otg.terraingen.noise;
...
public class NoiseGeneratorPerlin
{
    ...
    public void populateNoiseArray3D(double NoiseArray[], double xOffset, double yOffset, double zOffset, int xSize, int ySize, int zSize, double xScale, double yScale, double zScale, double noiseScale)
    {
        ...
        {
            d20 = xOffset + (double) i5 * xScale + xCoord;
            k5 = (int) d20;
            if (d20 < (double) k5)
            {
                k5--;
            ...
            i6 = k5 & 0xff;
            d20 -= k5;
            d22 = d20 * d20 * d20 * (d20 * (d20 * 6D - 15D) + 10D);
            for (int j6 = 0; j6 < zSize; j6++)
            {
                d24 = zOffset + (double) j6 * zScale + zCoord;
                k6 = (int) d24;
                if (d24 < (double) k6)
                {
                    k6--;
                ...
                l6 = k6 & 0xff;
                d24 -= k6;
                d25 = d24 * d24 * d24 * (d24 * (d24 * 6D - 15D) + 10D);
    ...
    public void populateNoiseArray2D(double NoiseArray[], double xOffset, double zOffset, int xSize, int zSize, double xScale, double zScale, double noiseScale)
    {
        ...
        {
            d14 = xOffset + (double) i4 * xScale + xCoord;
            j4 = (int) d14;
            if (d14 < (double) j4)
            {
                j4--;
            ...
            k4 = j4 & 0xff;
            d14 -= j4;
            d17 = d14 * d14 * d14 * (d14 * (d14 * 6D - 15D) + 10D);
            for (int l4 = 0; l4 < zSize; l4++)
            {
                d19 = zOffset + (double) l4 * zScale + zCoord;
                j5 = (int) d19;
                if (d19 < (double) j5)
                {
                    j5--;
                ...
                l5 = j5 & 0xff;
                d19 -= j5;
                d21 = d19 * d19 * d19 * (d19 * (d19 * 6D - 15D) + 10D);
                l = permutations[k4];
                j1 = permutations[l] + l5;
                k1 = permutations[k4 + 1];
                l1 = permutations[k1] + l5;
                d9 = lerp(d17, func_4110_a(permutations[j1], d14, d19), grad(permutations[l1], d14 - 1.0D, 0.0D, d19));
                d11 = lerp(d17, grad(permutations[j1 + 1], d14, 0.0D, d19 - 1.0D), grad(permutations[l1 + 1], d14 - 1.0D, 0.0D, d19 - 1.0D));
                d23 = lerp(d21, d9, d11);
